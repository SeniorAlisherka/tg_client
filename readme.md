...
3. Первый запуск будет заблокирован, нажмите готово.

![Not Opened](pictures/not_opened.png)

4. Чтобы обойти это **Настройки → Privacy & Security → Open Anyway**

![Security Check](pictures/security_check.png)

5. Если попросит установить rosetta, установите (ничего не весит почти).

6. После этого приложение откроется нормально.

# Как пользоваться 

1. Запускаем приложение, откроется терминал, ждем 10 секунд~
2. Сканируем QR, входим в свой тг
3. Заходим в супергруппы, выбираем группу, в которой мы хотим проверить отсутствие участников / присутствие ноунеймов
4. Если вашей группы нет в списке, значит это не супергруппа, админ должен зайти в группу через обычный тг, нажать edit, нажать topics и все. Потом в терминале надо вернуться назад (b) и снова зайти в супергруппы и группа уже будет в списке
5. Чтобы проверить отсутствующих/ноунеймов, все НУЖНЫЕ участники группы должны быть у вас в контактах ТГ.
6. В случае изменения групп, учеников, гугл таблцы, новый учебный год, ошибка в приложении и тд. писать главному разрабу @lisherka
...
- Создал venv.
- Создал `.env`.
- Полученные 2 файла (1 основной, другой alias) я засунул в `lib`.
...
- Добавил секреты из .env и materials/secrets в gh secrets
